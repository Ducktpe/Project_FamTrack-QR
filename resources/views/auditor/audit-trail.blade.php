                            ...
                            <tr class="audit-row"
                                ...
                                <td>
                                    ...
                                    @if($log->user)
                                        ...
                                        @endphp
                                        <span class="user-role-badge" style="display:inline-block;margin-top:3px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;padding:1px 7px;border-radius:3px;background:{{ $rc['bg'] }};color:{{ $rc['color'] }};border:1px solid {{ $rc['border'] }};">
                                            {{ ucfirst($log->user->role) }}
                                        </span>
                                    @elseif($log->user_name === 'System')
                                        <span class="user-role-badge" style="display:inline-block;margin-top:3px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;padding:1px 7px;border-radius:3px;background:#F3F4F6;color:#6B7280;border:1px solid #D1D5DB;">
                                            System
                                        </span>
                                    @endif
                                </td>
                                ...
                                <tr class="details-row" id="details-{{ $log->id }}">
                                    <td colspan="6">
                                        <div class="details-inner">
                                            ...
                                            @if($isDistrib && !empty($nv) && !empty($nv['status']) && $nv['status'] === 'cancelled')
                                                {{-- ── Cancellation details ── --}}
                                                @php
                                                    $cancelledAt = !empty($nv['cancelled_at'])
                                                        ? \Carbon\Carbon::parse($nv['cancelled_at'])->setTimezone('Asia/Manila')->format('M d, Y h:i A')
                                                        : null;
                                                @endphp
                                                <div style="display:flex;align-items:flex-start;gap:16px;">
                                                    <div style="flex-shrink:0;width:52px;height:52px;background:var(--red-pale);border:2px solid #FECACA;border-radius:8px;display:flex;align-items:center;justify-content:center;">
                                                        <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="var(--red)" stroke-width="1.8">
                                                    ...
                                                    <div style="flex:1;display:flex;flex-direction:column;gap:10px;">
                                                        <div style="background:var(--red-pale);border:1px solid #FECACA;border-left:3px solid var(--red);border-radius:4px;padding:10px 14px;">
                                                            <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--red);margin-bottom:4px;">Event Status</div>
                                                            <div style="font-size:14px;font-weight:700;color:var(--red);">Cancelled</div>
                                                        </div>

                                                        @if(!empty($nv['cancellation_reason']))
                                                            <div style="background:var(--white);border:1px solid var(--gray-200);border-radius:4px;padding:10px 14px;">
                                                                <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--gray-400);margin-bottom:5px;">Cancellation Reason</div>
                                                                <div style="font-size:13px;color:var(--gray-800);line-height:1.5;">{{ $nv['cancellation_reason'] }}</div>
                                                            </div>
                                                        @endif

                                                        <div style="background:var(--blue-pale);border:1px solid #C3D8F5;border-left:3px solid var(--blue);border-radius:4px;padding:8px 12px;display:flex;flex-wrap:wrap;gap:20px;">
                                                            @if($cancelledAt)
                                                                <div style="display:flex;align-items:center;gap:6px;font-size:11px;color:var(--blue-dark);">
                                                            ...
                                                            @if($log->user_name)
                                                                <div style="display:flex;align-items:center;gap:6px;font-size:11px;color:var(--blue-dark);">
                                                                    ...
                                                                    <strong>Cancelled by:</strong> {{ $log->user_name }}
                                                                </div>
                                                            @endif
                                                            @if($log->affected_name)
                                                                <div style="display:flex;align-items:center;gap:6px;font-size:11px;color:var(--blue-dark);">
                                                                    ...
                                                                    <strong>Event:</strong> {{ $log->affected_name }}
                                                                </div>
                                                            @endif
                                                        </div>
                                                    </div>
