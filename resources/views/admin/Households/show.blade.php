    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header-user-badge {
            ...
            border: 1px solid var(--gray-200); border-radius: 4px; flex-shrink: 0;
        }
        .user-avatar {
        ...
        /* ─── MAIN ─── */
        .main-content {
            grid-area: main; background: var(--gray-50); overflow-y: auto; padding: 28px 32px;
        }
        ...
        /* ─── ALERTS ─── */
        .alert-success {
            background: var(--green-pale); border: 1px solid #BBF7D0; border-left: 4px solid var(--green);
            padding: 12px 16px; margin-bottom: 16px; font-size: 13px; color: var(--green-dark);
            display: flex; align-items: center; gap: 10px;
        }
        .alert-success svg { width: 16px; height: 16px; flex-shrink: 0; }
        .alert-danger {
            background: var(--red-pale); border: 1px solid #FECACA; border-left: 4px solid var(--red);
            padding: 12px 16px; margin-bottom: 16px; font-size: 13px; color: var(--red);
        ...
        .serial-display {
            ...
            background: var(--blue-pale); border: 1px solid #C7D9F3;
        ...
        .qr-serial {
            ...
            color: var(--blue); letter-spacing: 1.5px; margin-bottom: 4px;
        }
                    ...
                    <div class="section-body">
                        <div class="info-grid">
                            ...
                            <div class="info-item">
                                ...
                                <div class="info-value">
                                    @if($household->listahanan_id)
                                        <span class="mono">{{ $household->listahanan_id }}</span>
                                    @else
                                        <em>Not enrolled</em>
                                    @endif
                                </div>
                    ...
                    <div class="actions-stack">
                        ...
                        @if(!$household->isApproved())
                            <form method="POST" action="{{ route('admin.households.approve', $household) }}">
                                ...
                                <button type="submit" class="btn-action btn-action-approve"
                                    onclick="return confirm('Approve this household?')">
                        ...
                        @else
                            <form method="POST" action="{{ route('admin.households.unapprove', $household) }}">
                                @csrf
                                <button type="submit" class="btn-action btn-action-unapprove"
                                    onclick="return confirm('Remove approval from this household?')">
                        ...
                        @if($household->isApproved() && !$household->qrCode)
                            <form method="POST" action="{{ route('admin.households.qr.generate', $household) }}">
                                @csrf
                                <button type="submit" class="btn-action btn-action-approve"
                                    onclick="return confirm('Generate QR code for this household?')"
